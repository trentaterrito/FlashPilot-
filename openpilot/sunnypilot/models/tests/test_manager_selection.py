...
class TestActiveBundleValidation:
  ...
  def valid_cache(self, tmp_path, monkeypatch):
    monkeypatch.setattr(helpers.Paths, "model_root", staticmethod(lambda: str(tmp_path)))
    for ref in ("small", "big", "gone", "other"):
      (tmp_path / f"{ref}.pkl").write_bytes(b"model")
  ...
  def _raw_bundle(ref: str, runner: int | None = None) -> dict:
    ...
    artifact = bundle.init("models", 1)[0].artifact
    artifact.fileName = f"{ref}.pkl"
    artifact.downloadUri.sha256 = sha256(b"model")
