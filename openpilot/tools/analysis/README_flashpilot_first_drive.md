...
python3 openpilot/tools/analysis/flashpilot_first_drive.py --compare \
  ...
  --route 'B2=/logs/b2-rlog.zst' \
  --route 'C-shadow=/logs/c-rlog.zst'
```

State definitions:

- **A:** path-angle ON, vision-only longitudinal.
- **B2:** path-angle ON, RB5T adapter ON, shadow OFF.
- **C-shadow:** B2 plus shadow diagnostics; published behavior remains B2.
