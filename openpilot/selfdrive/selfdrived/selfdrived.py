...
class SelfdriveD:
  ...
  def update_distance_button(self, CS):
    ...
    if self.CP.carFingerprint == FORD_CAR.FORD_F_150_LIGHTNING_MK1:
      ...
      if action == 'hold':
        if not self.params.get('FlashPilotFordExperimentalModeShortcut', return_default=True):
          return
  ...
  def update_alerts(self, CS):
    ...
    alerts = self.events.create_alerts(alert_types, [self.CP, CS, self.sm, self.is_metric, soft_disable_timer, pers])
