...
def calculate_cache_size() -> float:
  if not os.path.exists(CUSTOM_MODEL_PATH):
    return 0.0
  return sum(os.path.getsize(os.path.join(CUSTOM_MODEL_PATH, file))
             for file in os.listdir(CUSTOM_MODEL_PATH)) / (1024 ** 2)
...
class ModelsLayoutMici(NavScroller):
  ...
  def _update_state(self):
    ...
    self.current_model_info.info_text.set_text(f"{calculate_cache_size():.2f} MB")
