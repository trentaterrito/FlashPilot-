...
from openpilot.common.producer_state import card_state, observe_cruise_arguments
...
def can_comm_callbacks(logcan: messaging.SubSocket, sendcan: messaging.PubSocket, recorder=None) -> tuple[CanRecvCallable, CanSendCallable]:
  def can_recv(wait_for_one: bool = False) -> list[list[CanData]]:
    ...
    """
    ret = []
    can_events = messaging.drain_sock(logcan, wait_for_one=wait_for_one)
    if recorder is not None:
      recorder.can_batch(can_events, phase="can_callback")
...
class Car:
  ...
  def __init__(self, CI=None, RI=None) -> None:
    self.producer_recorder = Recorder("card")
    self.can_sock = messaging.sub_sock('can', timeout=20)
    self.sm = ObservedSubMaster(['pandaStates', 'carControl', 'onroadEvents'], recorder=self.producer_recorder)
    self.pm = ObservedPubMaster(['sendcan', 'carState', 'carParams', 'carOutput', 'radarTracks'],
                                recorder=self.producer_recorder, observed={'carState', 'carParams', 'radarTracks'})
    self.pm.sm = self.sm
    ...
    self.can_callbacks = can_comm_callbacks(self.can_sock, self.pm.sock['sendcan'], self.producer_recorder)
    ...
    if CI is None:
      ...
      while True:
        ...
        self.producer_recorder.can_batch([can], phase="startup_wait")
        if len(can.can) > 0:
          break
    ...
    observe_cruise_arguments(self, self.producer_recorder)
    ...
    self.producer_recorder.capture_state("initial", lambda: card_state(self))

  def state_update(self) -> tuple[car.CarState, structs.RadarDataT | None]:
    """carState update loop, driven by can"""

    can_strs = messaging.drain_sock_raw(self.can_sock, wait_for_one=True)
    self.producer_recorder.begin(self.sm)
    self.producer_recorder.can_batch(can_strs)
    self.producer_recorder.capture_state("before", lambda: card_state(self))
    ...
    self.producer_recorder.capture_state("after", lambda: card_state(self))
  ...
  def controls_update(self, CS: car.CarState, CC: car.CarControl):
    ...
    if not self.initialized_prev:
      ...
      self.producer_recorder.reset("car_interface.init_enter")
      self.CI.init(self.CP, *self.can_callbacks)
      self.producer_recorder.note("car_interface.init_return", True)
