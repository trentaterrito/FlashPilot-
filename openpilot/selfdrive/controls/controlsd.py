...
class Controls:
  def __init__(self) -> None:
    ...
    self.sm = messaging.SubMaster(['lateralDelay', 'vehicleParameters', 'lateralTorqueParameters', 'modelV2', 'selfdriveState',
                                   ...
                                   'driverMonitoringState', 'onroadEvents', 'driverAssistance', 'pandaStates', 'deviceState', 'radarState'], poll='selfdriveState')
    ...
    self.always_on_lateral = AlwaysOnLateralHost(self.CP.carFingerprint == "FORD_F_150_LIGHTNING_MK1")
  ...
  def state_control(self):
    ...
    lateral = self.always_on_lateral.update(
      onroad=self.sm['deviceState'].started, fresh=fresh,
    ...
    if lateral.session:
      CC.latActive = lateral.authorized and (not standstill or self.CP.steerAtStandstill)
    ...
    actuators.accel = float(self.LoC.update(CC.longActive, CS, long_plan.aTarget, long_plan.shouldStop,
                                           pid_accel_limits, long_plan, self.sm['radarState']))
  ...
  def publish(self, CC, lac_log):
    ...
    # Legacy schema names are retained as the proven transport to pandad/panda.
    # They no longer represent user-facing MADS state or physical-button intent.
    cs.madsState.enabled = self.always_on_lateral.result.eligible
    cs.madsState.active = self.always_on_lateral.result.authorized
    cs.madsState.available = self.always_on_lateral.result.session
    cs.madsAuthorized = self.always_on_lateral.result.authorized
    cs.madsEligible = self.always_on_lateral.result.eligible
    cs.madsSoftDisableTimer = 0
