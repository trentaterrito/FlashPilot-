...
class TestVisionStopReleaseGuard:
  ...
  pid requires vRel >= 0 continuously for VISION_LEAD_RELEASE_TICKS
  (150 ms at 100 Hz).

  NOTE: this baseline's long_control_state_trans() has no separate "starting"
  state (removed upstream vs. the old v0.11.1-based port); release always
  targets LongCtrlState.pid directly, which is what every assertion below
  already checks unmodified.
  """
