...
2. **Local capture during a bench/ignition-on session on the actual truck**, then hand-adding the bytes exactly as reported (this is the only path compatible with the "no fake firmware" guardrail). **Update:** the fingerprints.py header's `opendbc/car/debug/format_fingerprints.py` reference does not exist in our pinned opendbc submodule commit — the verified, working tool at our current baseline is `tools/scripts/fingerprint_from_route.py` in the openpilot superproject. See `FLASHLIGHTNING_FIRMWARE_CAPTURE.md` for the exact, source-verified procedure and required-vs-optional ECU breakdown.
...
2. **SecOC/TRON byte-length check** (`FLASHLIGHTNING_UPSTREAM_AUDIT.md` §5): whether `LateralMotionControl2`/`ACCDATA` are 8 or 16 bytes on our specific truck's camera bus is a hardware fact we cannot know until we sniff the bus. If 16 bytes, `dashcamOnly` triggers regardless of fingerprint correctness, and that is **out of scope to "fix"** here (it reflects a real Ford security architecture change, not a bug). **Update from vehicle owner:** this 2024 Lightning is reported to *not* use TRON/SecOC steering messaging — downgraded from an open blocker to something worth a quick empirical confirmation during capture (`FLASHLIGHTNING_FIRMWARE_CAPTURE.md` §5 shows exactly how), not an unknown.
