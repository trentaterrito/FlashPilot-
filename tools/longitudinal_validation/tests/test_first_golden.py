...
def test_regression_core_and_acceptance_exactly_preserved_from_pinned_baseline():
  ...
  # Explicit opt-in event/required-diagnostic branches have their own tests.
  # Removing only this validated extension must recover the original v1 AST.
  start = next(i for i,x in enumerate(body) if isinstance(x, ast.Assign) and
               any(isinstance(t, ast.Name) and t.id == 'contract' for t in x.targets))
  stop = next(i for i,x in enumerate(body) if isinstance(x, ast.Assign) and
              any(isinstance(t, ast.Name) and t.id == 'gates' for t in x.targets))
  extension = body[start:stop]
  assert len(extension) == 5
  assert isinstance(extension[1], ast.ImportFrom) and extension[1].module == 'events'
  assert ast.unparse(extension[2]) == 'event_comparison = None'
  assert ast.unparse(extension[3].test) == 'reviewed_policy(contract) != LEGACY'
  assert ast.unparse(extension[4].test) == "contract.get('required_diagnostics')"
  body = body[:start] + body[stop:]
  # Legacy return has no new fields; only opted-in v2 adds its explicit evidence.
  returned = body[-1].value
  unpack = returned.keys.index(None)
  assert ast.unparse(returned.values[unpack].test) == 'event_comparison is not None'
  assert ast.unparse(returned.values[unpack].orelse) == '{}'
  returned.keys.pop(unpack); returned.values.pop(unpack)
...
def test_measurement_diagnostic_error_is_rb5t_only_and_preserves_independent_output():
  ...
  assert snapshot['rb5t']['available'] is False
  assert snapshot['rb5t']['status'] == 'UNAVAILABLE_INCOMPLETE_CORROBORATION'
  assert snapshot['pass_fail_eligible'] is False
  error = snapshot['rb5t']['first_unavailable_contribution']['error']
  assert error['raw_hex'] == frame.data.hex() and error['t_ns'] == frame.t_ns
  assert error['layout_errors'][0]['shift'] == -6
  assert snapshot['ford_object']['fresh'] is True
  assert snapshot['rb5t']['support_present'] is None
  decoder.update([CanFrame(frame.t_ns+1, 0x3d7, bytes(8), 2)])
  assert decoder.snapshot(frame.t_ns+2)['ford_object']['t_ns'] == frame.t_ns+1
  assert decoder.snapshot(frame.t_ns+2)['rb5t']['support_present'] is None
...
def test_diagnostic_decoder_does_not_catch_unrelated_exception(monkeypatch):
  from tools.longitudinal_validation import diagnostics
  measurement = fg.MeasurementDiagnostics()
  monkeypatch.setattr(diagnostics, 'decode_ford_frame', Mock(side_effect=RuntimeError('unrelated failure')))
  with pytest.raises(RuntimeError, match='unrelated failure'):
    measurement.update([recorded_decode_failure()])


def test_measurement_and_strict_use_same_scoped_decoder():
  frame = recorded_decode_failure()
  strict, measurement = FordDiagnosticDecoder(), fg.MeasurementDiagnostics()
  strict.update([frame]); measurement.update([frame])
  assert strict.snapshot() == measurement.snapshot()
  assert strict.snapshot()['rb5t']['support_count'] is None
