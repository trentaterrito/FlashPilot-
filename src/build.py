...
subprocess.run(['clang','--target=aarch64-linux-gnu','-c','entry.S','-o',str(out/'entry.o')],cwd=root,check=True)
b=(out/'entry.o').read_bytes()
...
p=out/'flashpilot-dfd4b419-installer'; p.write_bytes(blob); p.chmod(0o755)
