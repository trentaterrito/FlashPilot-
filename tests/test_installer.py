...
active=sys.argv[1]=='-C' and sys.argv[2].startswith('/')
cmd=' '.join(args)
if args[:1]==['clone']:
 if case=='clone': sys.exit(1)
 p=pathlib.Path(args[-1]); p.mkdir(); (p/'launch_openpilot.sh').write_text('never execute'); (p/'launch_openpilot.sh').chmod(0o755)
...
if args[:2]==['rev-parse','HEAD']:
 ...
 path=sys.argv[2] if sys.argv[1]=='-C' else ''
